    ...
    def step(self, action, human):
        ...
        while not self.started:
            self.blit_text_center(
                self.WIN, self.MAIN_FONT, "Press any key to start level!")
            pygame.display.update()
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.close()
        ...
        return reward1, reward, done, finish, \
    [self.player_car.centerx, self.player_car.centery]
    
    def blit_text_center(self, win, font, text):
        render = font.render(text, 1, (0, 0, 0))
        win.blit(render, (win.get_width()/2 - render.get_width() /
                          2 + 150, win.get_height()/2 - render.get_height()/2 - 50))
...
#             pass
    ...
    def step(self, action):
        ...
        if self.player_car.crash :
            ...
            reward2 = self.rewardDistance()
            alpha = 1
            # reward = reward1 - alpha*(reward2**2)
            reward = reward1
        return next_state, reward, done
...
class car_environment_trajectory():
    ...
    def draw(self): #(win, images, player_car):
        ...
        angle_text = self.MAIN_FONT.render(
            "Human trajectory", 1, (255, 0, 0))
        self.WIN.blit(angle_text, (10, self.HEIGHT - angle_text.get_height() - 90))
        
        angle_text = self.MAIN_FONT.render(
            "AI trajectory", 1, (0,100,255))
        self.WIN.blit(angle_text, (10, self.HEIGHT - angle_text.get_height() - 60))
